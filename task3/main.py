...
def train(model, optimizer, train_loader, criterion, batch_size, epochs, scheduler=None, 
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (ima, imb, imc) in enumerate(train_loader):
            ...
            acc_epoch.update(acc, ima.shape[0])
            pbar.set_postfix_str("{}, accuracy {}/{}".format(losses, acc, ima.shape[0]))
            pbar.update()
        print(f"Epoch {epoch+1} ended with average loss {losses.avg} and overall accuracy {acc_epoch.avg:.3f}.")
        ...
        if val_loader:
            ...
            with torch.no_grad():
                ...
                print("\nValidation data average loss {}, accuracy {:.3f}.".format(losses_val.avg, curr_acc))
                if val_acc < curr_acc:
                    val_acc = curr_acc
                    best_epoch = epoch + 1
                    # if epoch < 3:
...
if __name__ == '__main__':
    ...
    if VALIDATION:
        ...
        valid_loader = DataLoader(validation_set, batch_size=BATCH_SIZE, shuffle=False, drop_last=False)
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.46416) if STEP_LR else None
    
    starting_epoch = 0
    if LOAD_MODEL:
        try:
            starting_epoch, model_pretrained, optimizer, scheduler = load_checkpoint(os.path.join(path, MODEL_PATH + MODEL_FILE_NAME), model_pretrained, optimizer, scheduler)
            print("Model loaded!")
        except:
    ...
    if TRAIN:
        train(model=model_pretrained, optimizer=optimizer, train_loader=train_loader, criterion=criterion, 
            batch_size=BATCH_SIZE, epochs=EPOCHS, scheduler=scheduler, model_name='model', val_loader=valid_loader, starting_epoch=starting_epoch)

...
if __name__ == '__main__':
    ...
    ptrain_t_set, ptrain_v_set = random_split(ptrain_set, [train_len, len(ptrain_set) - train_len])
    ptrain_loader = DataLoader(ptrain_set, batch_size=BATCH_SIZE,
                            shuffle=False,
                            drop_last=False,
                            )  # for all pretrain data, prediction
    ptrain_t_loader = DataLoader(ptrain_t_set, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)
    ptrain_v_loader = DataLoader(ptrain_v_set, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)

    outputs = train(model_AE, EPOCHS_AE, ptrain_t_loader, ptrain_v_loader, optimizer, scheduler, mode="AE")
    ...
    ptrain_reduced_set = MoleculeDataSet(data,
                                        ...
                                        mode="pretrain")
    ptrain_red_t_set, ptrain_red_v_set = random_split(ptrain_reduced_set, [train_len, len(ptrain_set) - train_len])
    ptrain_reduced_loader = DataLoader(ptrain_reduced_set, batch_size=BATCH_SIZE,
                                    shuffle=True,
                                    drop_last=False,
                                    )
    ptrain_red_t_loader = DataLoader(ptrain_red_t_set, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)
    ptrain_red_v_loader = DataLoader(ptrain_red_v_set, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)

    outputs = train(model_nn, EPOCHS_PT, ptrain_red_t_loader, ptrain_red_v_loader, optimizer, scheduler, mode="pretrain")
